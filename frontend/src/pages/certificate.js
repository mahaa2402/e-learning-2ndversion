...
const CertificatePage = () => {
  ...
  useEffect(() => {
    console.log('🪪 Certificate page mounted:', { initialCertificateData, savedCourseId: localStorage.getItem('certificateCourseId'), savedLessonId: localStorage.getItem('certificateLessonId') });
  ...
  useEffect(() => {
    const autoGeneratePdf = async () => {
      if (certificateData && !loading && !error && !autoGeneratedPdfUrl) {
        try {
          console.log('🔄 Auto-generating certificate PDF...');
          const pdfBytes = await generatePdfBytes();
          const blob = new Blob([pdfBytes], { type: 'application/pdf' });
          const blobUrl = window.URL.createObjectURL(blob);
          setAutoGeneratedPdfUrl(blobUrl);
          setPdfGenerated(true);
          console.log('✅ Certificate PDF auto-generated and displayed');
        } catch (err) {
          console.error('❌ Error auto-generating certificate PDF:', err);

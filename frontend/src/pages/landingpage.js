    ...
    const fetchCommonCourses = async () => {
      try {
        ...
        if (userRole === 'employee' && token) {
          console.log('👤 Employee detected - fetching assigned courses for dropdown');
          ...
          try {
            ...
            if (assignedResponse.ok) {
              ...
              console.log('📋 Assigned courses data for dropdown:', assignedData);
              ...
              console.log('📝 Assigned course names for dropdown:', assignedCourseNames);
              
              if (assignedCourseNames.length === 0) {
                console.log('ℹ️ No assigned courses found for employee dropdown');
                setCommonCourses([]);
                return;
              }
              ...
              if (allCoursesResponse.ok) {
                ...
                // Filter courses to only include assigned ones (case-insensitive match)
                const filteredCourses = allCourses.filter(course => {
                  const courseTitle = course.title || '';
                  const isAssigned = assignedCourseNames.some(assignedName => 
                    assignedName.toLowerCase() === courseTitle.toLowerCase()
                  );
                  return isAssigned;
                });
                
                console.log(`✅ Filtered ${filteredCourses.length} assigned course(s) for dropdown from ${allCourses.length} total course(s)`);
                console.log('📋 Filtered courses:', filteredCourses.map(c => c.title));
                setCommonCourses(filteredCourses);
              } else {
                console.error('❌ Failed to fetch all courses for filtering:', allCoursesResponse.status);
                setCommonCourses([]);
              }
            } else {
              console.error('❌ Failed to fetch assigned courses:', assignedResponse.status);
              setCommonCourses([]);
            }
          } catch (error) {
            console.error('❌ Error fetching assigned courses for dropdown:', error);
            setCommonCourses([]);
          }
        } else {
          // For admin or non-logged-in users, fetch all courses
          console.log('👤 Admin or guest - fetching all courses for dropdown');
